...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error: any, errorInfo: any) {
    console.error('ERP Module Error:', error, errorInfo);
  }
    ...
    if ((this.state as any).hasError) {
      return (
        <div className="p-6 text-center">
          <h2 className="text-2xl font-bold text-red-600 mb-4">모듈 로딩 오류</h2>
          <p className="text-gray-600 mb-4">ERP 모듈을 로딩하는 중 오류가 발생했습니다.</p>
          <details className="text-left bg-gray-100 p-4 rounded-lg">
            ...
            <pre className="mt-2 text-sm text-red-600 overflow-auto">
              {JSON.stringify((this.state as any).error, null, 2)}
            </pre>
          </details>
        </div>
      );
...
let DrawingsModule: any = null;
let SettingsModule: any = null;

try {
  const calendarModuleImport = require("./calendar-module");
  CalendarModule = calendarModuleImport.CalendarModule;
} catch (error) {
  console.error('Calendar module load error:', error);
}

try {
  const emailModuleImport = require("./email-module");
  EmailModule = emailModuleImport.EmailModule;
} catch (error) {
  console.error('Email module load error:', error);
}

try {
  const drawingsModuleImport = require("./drawings-module");
  DrawingsModule = drawingsModuleImport.DrawingsModule;
} catch (error) {
  console.error('Drawings module load error:', error);
}

try {
  const settingsModuleImport = require("./settings-module");
  SettingsModule = settingsModuleImport.SettingsModule;
} catch (error) {
  console.error('Settings module load error:', error);
}
        ...
        switch (activeTab.type) {
          case 'calendar':
            return CalendarModule ? (
              ...
              <div className="p-6 text-center">
                <h2 className="text-2xl font-bold text-gray-800 mb-4">캘린더 모듈 로딩 실패</h2>
                <p className="text-gray-600">캘린더 모듈을 불러올 수 없습니다.</p>
              </div>
            );
          case 'email':
            return EmailModule ? (
              ...
              <div className="p-6 text-center">
                <h2 className="text-2xl font-bold text-gray-800 mb-4">이메일 모듈 로딩 실패</h2>
                <p className="text-gray-600">이메일 모듈을 불러올 수 없습니다.</p>
              </div>
            );
          case 'drawings':
            return DrawingsModule ? (
              ...
              <div className="p-6 text-center">
                <h2 className="text-2xl font-bold text-gray-800 mb-4">도면 모듈 로딩 실패</h2>
                <p className="text-gray-600">도면 모듈을 불러올 수 없습니다.</p>
              </div>
            );
          case 'settings':
            return SettingsModule ? (
              ...
              <div className="p-6 text-center">
                <h2 className="text-2xl font-bold text-gray-800 mb-4">설정 모듈 로딩 실패</h2>
                <p className="text-gray-600">설정 모듈을 불러올 수 없습니다.</p>
              </div>
            );
          default:
